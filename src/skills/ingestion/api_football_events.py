"""APIFootballEventsExtractor — /fixtures/events → bronze.football_events_raw."""
...
_TABLE = "football_events_raw"
_ENDPOINT = "fixtures/events"


@register_skill
class APIFootballEventsExtractor(APIFootballBaseSkill):
    """Extrai gols, cartões e substituições para a Bronze."""

    @property
    def name(self) -> str:
        return "api_football_events_extractor"
    ...
    def execute(self, context: ExecutionContext) -> SkillResult:
        ...
        log.info("events_extraction_start", fixtures=len(fixture_ids))
        total = 0
        ...
        try:
            ...
            return SkillResult.ok(
                f"Extraídos {total} eventos de {len(fixture_ids)} fixtures",
                rows_affected=total,
            )
        except Exception as exc:
            log.error("events_extraction_failed", error=str(exc))
            return SkillResult.fail(f"Falha na extração de events: {exc}", error=exc)
