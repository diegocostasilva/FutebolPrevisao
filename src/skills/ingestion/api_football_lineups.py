"""APIFootballLineupsExtractor — /fixtures/lineups → bronze.football_lineups_raw."""
...
_TABLE = "football_lineups_raw"
_ENDPOINT = "fixtures/lineups"


@register_skill
class APIFootballLineupsExtractor(APIFootballBaseSkill):
    """Extrai escalações confirmadas para a Bronze."""

    @property
    def name(self) -> str:
        return "api_football_lineups_extractor"
    ...
    def execute(self, context: ExecutionContext) -> SkillResult:
        ...
        log.info("lineups_extraction_start", fixtures=len(fixture_ids))
        total = 0
        ...
        try:
            ...
            return SkillResult.ok(
                f"Extraídas escalações de {len(fixture_ids)} fixtures ({total} registros)",
                rows_affected=total,
            )
        except Exception as exc:
            log.error("lineups_extraction_failed", error=str(exc))
            return SkillResult.fail(f"Falha na extração de lineups: {exc}", error=exc)
